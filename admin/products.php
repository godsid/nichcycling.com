...
$productItem  = $product->getAdmin(false,$page,$limit=20);
							...
							<?php
								...
								?>
									<tr>
										...
										<td><a href="<?=ADMIN_URL.'/products_new.php?id='.$one['id']?>" title="<?=$one['code']?>"><?=$one['code']?></a></td>
										<td><?=number_format($one['price'])?></td>
										<td><strong>Color:</strong> <?=$one['color']?><br/><strong>Size:</strong> <?=$one['size']?></td>
										<td>
											<!-- Icons -->
											 <a href="<?=ADMIN_URL?>/products_new.php?id=<?=$one['id']?>" title="Edit"><img src="resources/images/icons/pencil.png" alt="Edit" /></a>
											 <a href="<?=WWW_URL?>/function/fn_product.php?action=delete&id=<?=$one['id']?>" onclick="return window.confirm('Are u sure to delete this item')" title="Delete"><img src="resources/images/icons/cross.png" alt="Delete" /></a> 
										</td>
									</tr>
								<?php
							    }
